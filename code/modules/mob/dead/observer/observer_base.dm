...
/obj/machinery/atmospherics/proc/ghost_gas_analyze(mob/dead/observer/user, atom/A)
	to_chat(world, "2")
...
/turf/CtrlClick(mob/dead/observer/user) //try to merge this proc sjdflokajhsfiuweq
	to_chat(world, "5")
	if(!istype(user))
		return
	if(user.gas_analyzer)
		...
		to_chat(user, "<span class='boldnotice'>Results:</span>")
		...
		if(total_moles)
			...
			if(abs(n2_concentration - N2STANDARD) < 20)
				to_chat(user, "<span class='notice'>Nitrogen: [round(n2_concentration * 100)]% ([round(environment.nitrogen, 0.01)] moles)</span>")
			else
				to_chat(user, "<span class='warning'>Nitrogen: [round(n2_concentration * 100)]% ([round(environment.nitrogen, 0.01)] moles)</span>")

			if(abs(o2_concentration - O2STANDARD) < 2)
				to_chat(user, "<span class='notice'>Oxygen: [round(o2_concentration * 100)]% ([round(environment.oxygen, 0.01)] moles)</span>")
			else
				to_chat(user, "<span class='warning'>Oxygen: [round(o2_concentration * 100)]% ([round(environment.oxygen, 0.01)] moles)</span>")

			if(co2_concentration > 0.01)
				to_chat(user, "<span class='warning'>CO2: [round(co2_concentration * 100)]% ([round(environment.carbon_dioxide, 0.01)] moles)</span>")
			else
				to_chat(user, "<span class='notice'>CO2: [round(co2_concentration * 100)]% ([round(environment.carbon_dioxide, 0.01)] moles)</span>")

			if(plasma_concentration > 0.01)
				to_chat(user, "<span class='warning'>Plasma: [round(plasma_concentration * 100)]% ([round(environment.toxins, 0.01)] moles)</span>")

			if(n2o_concentration > 0.01)
				to_chat(user, "<span class='warning'>N2O: [round(n2o_concentration * 100)]% ([round(environment.sleeping_agent, 0.01)] moles)</span>")

			if(unknown_concentration > 0.01)
				to_chat(user, "<span class='warning'>Unknown: [round(unknown_concentration * 100)]% ([round(unknown_concentration * total_moles, 0.01)] moles)</span>")

			to_chat(user, "<span class='notice'>Temperature: [round(environment.temperature - T0C, 0.1)]&deg;C</span>")
			to_chat(user, "<span class='notice'>Heat Capacity: [round(environment.heat_capacity(), 0.1)]</span>")
